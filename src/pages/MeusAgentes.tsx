...
export default function MeusAgentes() {
  ...
  const loadAgents = async () => {
    ...
    // Get agents accessible via role + collaborator overrides
    ...
    // Merge: start with role access, then apply overrides
  ...
  const sendMessage = async () => {
    ...
    if (error) toast.error("Erro ao enviar mensagem");
      ...
      const assistantMsg: Message = {
        ...
        role: "assistant",
        content: `Recebi sua mensagem: "${content}". (Integração com IA em breve)`,
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <Card><CardContent className="py-12 text-center text-muted-foreground">Nenhum agente disponível para você</CardContent></Card>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {agents.map(agent => (
              <Card
                key={agent.id}
                className="cursor-pointer hover:shadow-lg transition-all hover:-translate-y-1 border bg-card/80 backdrop-blur-sm"
